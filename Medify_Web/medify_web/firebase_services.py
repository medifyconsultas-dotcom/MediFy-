...
# Cache global para permitir invalidação seletiva
_cache_storage = {}

def _timed_cache(ttl=30, cache_key_prefix=None):
    """Cache com TTL e opção de invalidação por prefixo."""
    def decorator(fn):
        cache = {}
        fn_name = fn.__name__
        prefix = cache_key_prefix or fn_name
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                cache[key] = (now, val)
                # Armazenar referência para invalidação
                if prefix not in _cache_storage:
                    _cache_storage[prefix] = cache
            except Exception:
                pass
            return val
        return wrapper
...
def invalidar_cache_consultas_paciente(paciente_uid=None):
    """Invalida o cache de consultas para um paciente específico ou todos."""
    try:
        if paciente_uid:
            # Invalidar apenas para o paciente específico
            # O cache usa (args, kwargs) como chave, então precisamos limpar todas as chaves relacionadas
            for prefix, cache in _cache_storage.items():
                if 'listar_consultas_completas_paciente' in prefix:
                    # Limpar todo o cache desta função (mais simples e seguro)
                    cache.clear()
        else:
            # Invalidar todo o cache de consultas
            for prefix, cache in _cache_storage.items():
                if 'consultas' in prefix.lower():
                    cache.clear()
    except Exception:
        pass
...
def listar_medicos_clinicas():
    ...
    medicos = []
    try:
        # OTIMIZAÇÃO: Filtrar no Firestore ao invés de carregar tudo e filtrar no código
        # Isso reduz drasticamente a quantidade de dados transferidos
        docs = db.collection('funcionarios').where('cargo', '==', 'medico').limit(200).stream()
        for doc in docs:
...
@_timed_cache(ttl=30, cache_key_prefix='listar_consultas_completas_paciente')  # Cache curto (30s) para consultas - dados mudam frequentemente
def listar_consultas_completas_paciente(paciente_uid: str = None, paciente_email: str = None, paciente_nome: str = None):
    ...
    # OTIMIZAÇÃO: Adicionar limites para evitar queries muito grandes
    # top-level by uid
    try:
        if paciente_uid:
            docs = db.collection('consultas').where('paciente_uid', '==', paciente_uid).limit(500).stream()
            for doc in docs:
                sd = _safe_snapshot_dict(doc)
                if sd:
                    add_consulta(sd)
    ...
    try:
        if paciente_email:
            docs = db.collection('consultas').where('paciente_email', '==', paciente_email).limit(500).stream()
            for doc in docs:
                sd = _safe_snapshot_dict(doc)
                if sd:
                    add_consulta(sd)
    ...
    # OTIMIZAÇÃO: Batch loading para evitar N+1 queries
    # Coletar todos os UIDs únicos primeiro
    profissional_uids = set()
    paciente_uids = set()
    for c in results:
        if not c.get('profissional_nome') and c.get('profissional_uid'):
            profissional_uids.add(c.get('profissional_uid'))
        if not c.get('paciente_nome') and c.get('paciente_uid'):
            paciente_uids.add(c.get('paciente_uid'))
    
    # Carregar todos os profissionais e pacientes de uma vez
    profissionais_cache = {}
    pacientes_cache = {}
    
    for uid in profissional_uids:
        try:
            p = obter_profissional(uid)
            if p and p.get('nome'):
                profissionais_cache[uid] = p.get('nome')
        except Exception:
            pass
    
    for uid in paciente_uids:
        try:
            pac = obter_paciente(uid)
            if pac and pac.get('nome'):
                pacientes_cache[uid] = pac.get('nome')
        except Exception:
            pass
    
    # Enriquecer usando o cache (sem queries adicionais)
    enriched = []
    for c in results:
        # populate profissional_nome do cache
        if not c.get('profissional_nome') and c.get('profissional_uid'):
            c['profissional_nome'] = profissionais_cache.get(c.get('profissional_uid'))
        # populate paciente_nome do cache
        if not c.get('paciente_nome') and c.get('paciente_uid'):
            c['paciente_nome'] = pacientes_cache.get(c.get('paciente_uid'))
        enriched.append(c)
