    ...
    try:
        ...
        if upcoming:
            ...
            for dt, c in upcoming[:3]:
                ...
                # Try to get professional/clinic info
                if profissional_uid:
                    # Check if it's a clinic employee (funcionario)
                    id_clinica = c.get('id_clinica') or c.get('clinica_uid') or c.get('clinica')
                    if id_clinica:
                        try:
                            # Get clinic address and phone
                            clinica = obter_clinica(id_clinica)
                            if clinica:
                                endereco = clinica.get('endereco', '')
                                telefone = clinica.get('telefone', '')
                            # Try to get employee phone from funcionarios collection
                            try:
                                funcionario_doc = db.collection('funcionarios').document(profissional_uid).get()
                                if funcionario_doc.exists:
                                    funcionario_data = funcionario_doc.to_dict()
                        ...
                        # It's an autonomous professional
                        try:
                            profissional = obter_profissional(profissional_uid)
                            if profissional:
...
def ver_agenda(request):
    ...
    # Ensure each consultation has required fields with defaults
    from .firebase_services import obter_profissional, obter_clinica, db
    formatted_consultas = []
